CEIR-OS unterstützt FHIR-Mapping-Workflows für die Transformation und Zuordnung medizinischer Daten. Diese Seite beschreibt die verfügbaren Methoden und deren Integration mit den Terminologie-Tools.

### Übersicht der Mapping-Methoden
...
| MapQual | Qualitätsberichte | Bewertung der Mapping-Qualität |

### ConceptMap für Terminologie-Mappings
...
| `#inexact` | Ungefähre Entsprechung |
| `#unmatched` | Keine Entsprechung |

### StructureMap für strukturelle Transformationen
...
### MapQual: Mapping-Qualitätssicherung

MapQual ist ein Ansatz zur systematischen Bewertung der Qualität von Terminologie-Mappings. Qualitätskriterien umfassen:

| Kriterium | Beschreibung |
|-----------|-------------|
| Vollständigkeit | Anteil der Quell-Codes mit Mapping |
| Korrektheit | Stimmt die semantische Zuordnung? |
| Granularität | Passt der Detailgrad (wider/narrower)? |
| Konsistenz | Einheitliche Anwendung der Equivalence-Typen |
| Aktualität | Basiert das Mapping auf aktuellen Versionen? |

### Integration mit Terminologie-Tools

Die Terminologie-Tools von CEIR-OS unterstützen den Mapping-Workflow:
...
| Code validieren | `validate_code` | Existenz im Zielsystem prüfen |
| LOINC-Zuordnung | `search_common_loinc` | Passenden LOINC-Code finden |
| Deutsche Labels | `get_german_label` | Übersetzung für Dokumentation |
...
2. **SNOMED-Kandidaten finden**: `lookup_code` mit System `http://snomed.info/sct` für verwandte Konzepte
...
5. **Qualität prüfen**: Equivalence-Typ und Vollständigkeit bewerten
